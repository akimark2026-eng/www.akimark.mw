// admin-manage.js (updated: uses video-upload-api, table videos)
...
(function() {
  ...
  existingFilmSelect.addEventListener('change', async function() {
    ...
    const { data, error } = await supabase.from('videos').select('*').eq('id', filmId).single();
    if (error) {
  ...
  form.addEventListener('submit', async function(e) {
    ...
    // 2. Upload each video and create video records
    let uploadedCount = 0;
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      try {
        const res = await fetch(`${SUPABASE_URL}/functions/v1/video-upload-api?action=get_upload_url`, {
          method: 'POST',
          headers: {
      ...
      const createRes = await fetch(`${SUPABASE_URL}/functions/v1/video-upload-api?action=create`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + token
