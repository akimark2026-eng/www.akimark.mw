...
(function() {
  ...
  const superAdmin = JSON.parse(localStorage.getItem('akmark_super_admin') || '{}');
  ...
  // Load current balance (optional - show in UI)
  async function loadBalance() {
    try {
      ...
      if (data.success) {
        ...
        if (balanceEl) {
          balanceEl.textContent = 'MWK ' + Number(data.total_revenue).toLocaleString('en-US', { minimumFractionDigits: 2 });
        }
      }
    } catch (e) {
